...
for group in $GATE_GROUPS; do
  case "$group" in
    ...
    geopt_transport_lifted)
      if [[ ! -e "$PRETRAIN_GEOPT_TRANSPORT/best_model.pt" ]]; then
        echo "Missing pretrained checkpoint for ${group}: $PRETRAIN_GEOPT_TRANSPORT/best_model.pt" >&2
        exit 1
      fi
      ;;
    geopt_original)
      if [[ ! -e "$PRETRAIN_GEOPT_ORIGINAL/$PRETRAIN_GEOPT_ORIGINAL_CHECKPOINT" ]]; then
        echo "Missing original GeoPT checkpoint: $PRETRAIN_GEOPT_ORIGINAL/$PRETRAIN_GEOPT_ORIGINAL_CHECKPOINT" >&2
        exit 1
      fi
      ;;
    *)
      echo "Unknown GATE_GROUPS entry: $group" >&2
      exit 1
      ;;
  esac
done
...
run_train() {
  ...
  PRETRAIN_GEOPT_ORIGINAL="$PRETRAIN_GEOPT_ORIGINAL" \
  PRETRAIN_GEOPT_ORIGINAL_CHECKPOINT="$PRETRAIN_GEOPT_ORIGINAL_CHECKPOINT" \
  ...
  GEOPT_ORIGINAL_NORMALIZATION_PROTOCOL="$GEOPT_ORIGINAL_NORMALIZATION_PROTOCOL" \
  GEOPT_ORIGINAL_NORMALIZATION_CONFIG="$GEOPT_ORIGINAL_NORMALIZATION_CONFIG" \
