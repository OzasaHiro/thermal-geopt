...
D1_MANIFEST="${D1_MANIFEST:-data/downstream_npz/d1_openfoam_complex_heatsink_m5_300/manifest.json}" \
BASE_SPLIT="${BASE_SPLIT:-configs/d1_openfoam_complex_heatsink_m5_300_split_seed42.json}" \
GATE_SPLIT_PATTERN="${GATE_SPLIT_PATTERN:-outputs/logs/m5_splits/d1_openfoam_complex_heatsink_m5_300_label_scarcity_seed{split_seed}.json}" \
...
RUN_PREFIX="${RUN_PREFIX:-m5_complex_heatsink_r4_vs_original}" \
SUMMARY_JSON="${SUMMARY_JSON:-outputs/logs/m5_complex_heatsink_transfer_summary.json}" \
SUMMARY_MD="${SUMMARY_MD:-docs/m5_complex_heatsink_transfer_results.md}" \
SUMMARY_TITLE="${SUMMARY_TITLE:-M5 Complex Heat-Sink Solver-Backed D1 Transfer Gate Results}" \
SUMMARY_DESCRIPTION="${SUMMARY_DESCRIPTION:-This report summarizes more complex solver-backed heat-sink D1 label-scarcity transfer runs comparing scratch, Thermal GeoPT pretraining, and the original GeoPT checkpoint.}" \
SUMMARY_INTERPRETATION_RULE="${SUMMARY_INTERPRETATION_RULE:-A positive M5 signal means a pretrained group improves scratch by about 10% relative L2 at 25 or 50 labels and does not degrade max-temperature or hotspot metrics. Original GeoPT is the primary cross-physics transfer control.}" \
DEVICE="${DEVICE:-cuda}" \
